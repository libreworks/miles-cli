...
class CognitoAuthAdapter extends AuthAdapter {
  ...
  /**
   * Creates a new CognitoUserPool.
   *
   * @private
   * @param {CognitoStorage} clientStorage - The credentials storage map.
   * @return {@amazon-cognito-identity-js.CognitoUserPool} The new user pool
   */
  createCognitoUserPool(clientStorage) {
   ...
   *
   * @private
   * @return {CognitoStorage} The credentials storage map
   */
  createCognitoStorage() {
    return CognitoStorage.create(this[SECRET_SERVICE]);
  }
  ...
  get cognitoFacade() {
    if (!this[FACADE]) {
      const clientStorage = this.createCognitoStorage();
      const userPool = this.createCognitoUserPool(clientStorage);
      this[FACADE] = new CognitoFacade(userPool, clientStorage);
      const saveSecrets = () => {
        clientStorage.store(this[SECRET_SERVICE]).then(() => {});
      };
