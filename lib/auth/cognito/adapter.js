...
class CognitoAuthAdapter extends AuthAdapter {
  ...
  get cognitoFacade() {
    if (!this[FACADE]) {
      const clientStorage = CognitoStorage.create(this[SECRET_SERVICE]);
      ...
      const self = this;
      const saveSecrets = () => {
        clientStorage.store(self[SECRET_SERVICE]).then(() => {});
      };
   ...
   */
  logIn() {
    return new CognitoLoginChallenge(this[FACADE]);
  }
