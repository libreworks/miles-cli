...
const { Prompt, createEmptyValidator } = require("../../io/prompt");
...
class CognitoChallenge extends Challenge {
   ...
   *
   * @param {Object} result - The raw result from the Cognito facade
   * @return {Result} An authentication result
   */
  processCognitoResult(result) {
    ...
    if (!result.incomplete) {
      ...
      if (result.name === "NEW_PASSWORD_REQUIRED") {
        ...
        return new CognitoPasswordResetChallenge(this[FACADE], cognitoUser);
      } else {
        throw new Error(`Unknown challenge type: ${result.name}`);
      }
    } else {
      throw new Error("Unknown result: neither complete nor a challenge");
    }
  }
}
...
class CognitoLoginChallenge extends CognitoChallenge {
  ...
  async complete(response) {
    ...
    return this.processCognitoResult(result);
  }
}
...
class CognitoPasswordChallenge extends CognitoChallenge {
   ...
   */
  async prompt(inputService, outputService) {
    const validator = createEmptyValidator("Password cannot be blank");
...
class CognitoPasswordResetChallenge extends CognitoPasswordChallenge {
  ...
  async prompt(inputService, outputService) {
    ...
    const validator = createEmptyValidator("Verification code cannot be blank");
    const prompt = new Prompt({
      name: "verification-code",
      intro: "Verification Code",
      validator,
    });
    outputService.write("A verification code was sent to you");
