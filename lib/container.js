...
class Provider {
  ...
  async provide(container) {
    if (this[INSTANCE] === undefined) {
      ...
      this[INSTANCE] = Promise.resolve(this[FACTORY](container));
      await this[INSTANCE];
      profiler.done({
        level: "debug",
        message: `Component instantiated: ${this[NAME]}`,
      });
...
class Container extends EventTarget {
  ...
  async get(name) {
    ...
    return await this[PROVIDERS].get(name).provide(this);
  }
  ...
  async getAll(names) {
    ...
    return await Promise.all(
      Array.from(names, async (name) => await this.get(name))
    );
  }
...
class Builder {
   ...
   * @return {bool} Whether the component factory is present.
   */
  has(name) {
    return this[PROVIDERS].has(name);
  }
  ...
  async build() {
    ...
    if (this[EAGER].size > 0) {
      ...
      profiler.done({
        level: "debug",
        message: "Instantiated eager components",
      });
    }
    this.reset();
    return container;
