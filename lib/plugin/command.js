...
class PluginCommand extends AbstractCommand {
   ...
   */
  constructor(logger, outputService, pluginService, npm) {
    super();
    this[LOGGER] = logger;
  ...
  static async create(container) {
    const [logger, outputService, pluginService] = await container.getAll([
      "logger",
      "io.output-service",
      "plugin.service",
    ]);
    return new PluginCommand(logger, outputService, pluginService);
  }
